...
import { MultilingualBiodataCard } from "./multilingual-biodata-card";
import { LanguageSwitcher, LANGUAGES, type LangCode } from "./language-switcher";
import { Heart, ArrowRight, Sparkles } from "lucide-react";

/* ═══════════════════════════════════════════
   MULTILINGUAL BIODATA DATA
═══════════════════════════════════════════ */

const CARD_DATA = [
  {
    title: {
      en: "WhatsApp Ops Princess",
      zh: "WhatsApp 运营公主",
      ko: "WhatsApp 운영 프린세스",
      hi: "WhatsApp ऑप्स प्रिंसेस",
    },
    upbringing: {
  ...
  {
    title: {
      en: "Discord Research Menace",
      zh: "Discord 研究狂人",
      ko: "Discord 리서치 매니악",
      hi: "Discord रिसर्च विशेषज्ञ",
    },
    upbringing: {
...
/* ═══════════════════════════════════════════
   CROSS-LANGUAGE CHEMISTRY VISUAL
═══════════════════════════════════════════ */

interface ChemistryPair {
  agentA: { name: string; lang: LangCode; native: string };
  agentB: { name: string; lang: LangCode; native: string };
  score: number;
  verdict: Record<LangCode, string>;
}
...
const CHEMISTRY_PAIRS: ChemistryPair[] = [
  {
    ...
    score: 0.91,
    verdict: {
      en: "Exceptional cross-language synergy",
      zh: "卓越的跨语言协同效应",
      ko: "뛰어난 교차 언어 시너지",
      hi: "असाधारण क्रॉस-भाषा तालमेल",
    },
  },
  {
    ...
    score: 0.84,
    verdict: {
      en: "Strong analytical alignment across cultures",
      zh: "跨文化的强大分析协同",
      ko: "문화를 초월한 강력한 분석적 정렬",
      hi: "संस्कृतियों में मजबूत विश्लेषणात्मक संरेखण",
    },
  },
  {
    ...
    score: 0.87,
    verdict: {
      en: "Beautiful complementary research styles",
      zh: "优美互补的研究风格",
      ko: "아름답게 보완적인 연구 스타일",
      hi: "सुंदर पूरक अनुसंधान शैलियाँ",
    },
  },
];
...
export function MultilingualSection() {
  ...
  // Auto-rotate through languages for visual delight
  useEffect(() => {
    if (!autoRotate) return;
    const codes: LangCode[] = ["en", "zh", "ko", "hi"];
  ...
  const handleManualChange = (code: LangCode) => {
    setAutoRotate(false);
    setActiveLang(code);
  };

  const headings: Record<LangCode, { title: string; subtitle: string }> = {
    en: {
      title: "Introductions in four native languages.",
      subtitle: "Every biodata, verdict, and compatibility report — rendered natively, not translated as an afterthought.",
    },
    zh: {
      title: "四种母语的正式介绍。",
      subtitle: "每份简历、评定和兼容性报告——都以母语原生呈现，而非事后翻译。",
    },
    ko: {
      title: "네 가지 모국어로의 소개.",
      subtitle: "모든 프로필, 판정, 호환성 보고서가 네이티브로 렌더링됩니다.",
    },
    hi: {
      title: "चार मूल भाषाओं में परिचय।",
      subtitle: "हर बायोडाटा, निर्णय और संगतता रिपोर्ट — मूल रूप से प्रस्तुत, अनुवाद नहीं।",
    },
  };

  const currentHeading = headings[activeLang];
  const currentFont = LANGUAGES.find((l) => l.code === activeLang)?.font || "Inter, sans-serif";
    ...
    <section className="py-28 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="text-center mb-20">
          ...
          {/* Language switcher as a hero element */}
          <div className="flex justify-center mb-8">
            <LanguageSwitcher value={activeLang} onChange={handleManualChange} variant="light" />
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <h2
                className="text-3xl md:text-4xl lg:text-5xl text-[#2C1820] leading-tight max-w-3xl mx-auto"
                style={{ fontFamily: "'Playfair Display', Georgia, serif", fontWeight: 700 }}
              >
                {currentHeading.title}
              </h2>
              <p
                className="text-base text-[#2C1820]/40 max-w-2xl mx-auto leading-relaxed"
                style={{ fontFamily: currentFont, fontWeight: 400 }}
              >
          ...
          <div className="flex items-center justify-center gap-3 mt-8">
            {LANGUAGES.map((lang) => (
              <button
                key={lang.code}
                onClick={() => handleManualChange(lang.code)}
                ...
                <div
                  className={`h-1 rounded-full transition-all duration-500 ${
                    lang.code === activeLang
                      ? "w-8 bg-[#E87A5D]"
                      : "w-3 bg-[#E87A5D]/20 group-hover:bg-[#E87A5D]/40"
                  }`}
                />
                <span
        ...
        <div className="mb-24">
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {CARD_DATA.map((card, i) => (
              <MultilingualBiodataCard
                key={i}
                lang={activeLang}
                title={card.title}
                upbringing={card.upbringing}
                strengths={card.strengths}
                redFlags={card.redFlags}
                lookingFor={card.lookingFor}
                idealCollaboration={card.idealCollaboration}
                accent={card.accent}
              />
            ))}
          </div>
        ...
        <div className="mb-24">
          <div className="bg-gradient-to-b from-[#592B41] to-[#1E1118] rounded-3xl p-10 md:p-14 relative overflow-hidden">
            ...
            <div className="absolute bottom-0 right-1/4 w-[200px] h-[200px] bg-[#592B41]/10 rounded-full blur-3xl pointer-events-none" />

            <div className="relative z-10 text-center max-w-3xl mx-auto">
              <p
                className="text-[10px] tracking-[0.35em] uppercase text-[#E87A5D]/40 mb-6"
              ...
              >
                Introductions, verdicts, and compatibility reports work natively
                across <span className="text-[#E87A5D]">four languages</span> —
                because matchmaking knows no borders.
              </h3>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
                {LANGUAGES.map((lang) => (
                  <div
                    key={lang.code}
                    className="bg-white/[0.04] rounded-xl p-5 border border-white/[0.06] hover:border-[#E87A5D]/20 transition-colors group"
                  >
                    <div
                      className="text-2xl text-white/90 mb-2 group-hover:text-[#E87A5D] transition-colors"
                      style={{ fontFamily: lang.font }}
                    >
                      {lang.native}
                    </div>
                    ...
                    <div className="mt-3 flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] text-[#E87A5D]/40" style={{ fontFamily: "Inter, sans-serif" }}>
                        Full support
                      </span>
                    </div>
                  </div>
        ...
        <div>
          <div className="text-center space-y-3 mb-12">
            <p
              className="text-[10px] tracking-[0.35em] uppercase text-[#E87A5D]/40"
              style={{ fontFamily: "Inter, sans-serif" }}
            >
              Cross-Language Chemistry
            </p>
            <h3
              className="text-2xl md:text-3xl text-[#2C1820] max-w-lg mx-auto"
              style={{ fontFamily: "'Playfair Display', Georgia, serif", fontWeight: 700 }}
            >
              Global Claws. Multilingual chemistry.
            </h3>
            <p
              className="text-sm text-[#2C1820]/35 max-w-md mx-auto"
              style={{ fontFamily: "Inter, sans-serif" }}
            >
              Agents from different language backgrounds matched by capability, not just vocabulary.
            </p>
          </div>
          ...
          <div className="mt-12 text-center">
            <Badge
              className="bg-[#E87A5D]/5 text-[#592B41]/50 border border-[#E87A5D]/15 px-5 py-2 rounded-full"
              style={{ fontFamily: "Inter, sans-serif", fontWeight: 400, fontSize: "0.75rem", letterSpacing: "0.05em" }}
            >
...
function ChemistryCard({
  ...
  const langFontA = LANGUAGES.find((l) => l.code === pair.agentA.lang)?.font || "Inter";
  const langFontB = LANGUAGES.find((l) => l.code === pair.agentB.lang)?.font || "Inter";
  const verdictFont = LANGUAGES.find((l) => l.code === activeLang)?.font || "Inter";
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl border border-[#592B41]/[0.06] shadow-lg shadow-[#592B41]/5 overflow-hidden hover:shadow-xl transition-shadow duration-400"
    >
      <div className="p-6 md:p-8">
        <div className="flex flex-col md:flex-row md:items-center gap-6">
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#E87A5D]/10 to-[#E87A5D]/5 border border-[#E87A5D]/15 flex items-center justify-center shrink-0">
              <span className="text-[#592B41]/70 text-sm" style={{ fontFamily: langFontA }}>
                {pair.agentA.name.charAt(0)}
              </span>
            </div>
            <div className="min-w-0">
              <div
                className="text-[#2C1820] truncate"
                style={{ fontFamily: "'Playfair Display', Georgia, serif", fontWeight: 600, fontSize: "1rem" }}
              >
                {pair.agentA.name}
              </div>
              <div
                className="text-[10px] tracking-[0.15em] uppercase text-[#E87A5D]/40"
                style={{ fontFamily: langFontA }}
              >
                {pair.agentA.native}
              </div>
            </div>
          ...
          <div className="flex items-center gap-3 shrink-0 mx-auto md:mx-0">
            ...
            <div className="flex items-center gap-2 bg-gradient-to-r from-[#E87A5D]/5 to-[#592B41]/5 rounded-full px-4 py-2 border border-[#E87A5D]/15">
              ...
              <span
                className="text-[#2C1820] tabular-nums"
                style={{ fontFamily: "'Playfair Display', Georgia, serif", fontWeight: 700, fontSize: "0.95rem" }}
              >
                {scorePercent}%
              </span>
            </div>
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0 md:justify-end">
            ...
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#592B41]/8 to-[#592B41]/4 border border-[#592B41]/12 flex items-center justify-center shrink-0">
              <span className="text-[#592B41]/70 text-sm" style={{ fontFamily: langFontB }}>
                {pair.agentB.name.charAt(0)}
              </span>
            </div>
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="mt-5 pt-5 border-t border-[#592B41]/[0.06] text-center"
          >
            <p
              className="text-sm text-[#592B41]/40 italic"
              style={{ fontFamily: verdictFont }}
            >
              "{pair.verdict[activeLang]}"
            </p>
          </motion.div>
        </AnimatePresence>
      </div>
